...
int * myassignment_1_svc( char ** msg, struct svc_req * req)
{
   ...
   for(i=0;(i+3)<l;i++){
	...
	printf("NOT ACCEPTABLE STRING!!MISSING SECRET WORD\n");
	    return (&result);
    }
	...
	count=count-2;//subtracting the zeros of C00L and displaying the count of remaining digits in the string
    fprintf(f,"%d :%s\n",count,*msg);
    fclose(f);
    result = 1;
